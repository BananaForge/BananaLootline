/* ===========================================================================
   BananaLootline - Fehlende Daten nachholen
   ...
   WAS GEHOLT WIRD
     Ringe      - vollstaendig. Der bisherige Lauf war bei 500 gekappt.
     Umhaenge   - dito, lag mit 498 hart an der Grenze.
     Amulette   - erneut, wegen der Use-Effekte (s.u.)
     Schmuck    - erneut, dort sind Use-Effekte die Regel
     Off-hand Frills und Relikte (Idole, Libramme, Totems) - fehlten ganz

   NEU: USE- UND EQUIP-EFFEKTE
     Werden jetzt mit ausgelesen, samt Wert, Dauer und Abklingzeit -
     getrennt, nicht als ein Text. Grund: "+100 Angriffskraft fuer 20 Sek
     alle 2 Min" sind im Schnitt rund 16 Angriffskraft, nicht 100. Nur mit
     Dauer und Abklingzeit kann das Addon das spaeter fair verrechnen.

   NEU: GEKAPPTE LISTEN
     Die Seite liefert hoechstens 500 Zeilen. Wird das erreicht, teilt das
     Skript den Slot selbst nach Qualitaet auf. Die Filtersyntax dieses
     Forks kenne ich nicht, deshalb probiert es mehrere Kandidaten durch
     und prueft am Ergebnis, welcher wirklich funktioniert.

   WAEHREND ES LAEUFT (optional)
     BLL.status()   Stand      BLL.save()   jetzt herunterladen
     BLL.stop()     abbrechen

   Tab offen lassen. Neuladen ist ok, Schliessen loescht den Zwischenstand.
=========================================================================== */
...
(function () {
  ...
  var SLOTS = (typeof BLL_SLOTS !== "undefined" && BLL_SLOTS.length) ? BLL_SLOTS : [
    "4.0.11",   /* Ringe - war gekappt */
    "4.1.16",   /* Umhaenge - war an der Grenze */
    "4.0.12",   /* Schmuck - wegen Use-Effekten */
    "4.0.2",    /* Amulette - wegen Use-Effekten */
    "4.0.23",   /* Off-hand Frills */
    "4.0.28"    /* Relikte: Idole, Libramme, Totems */
  ];
  ...
  function parseItemPage(html, id, slot) {
    ...
    /* Fuer die FESTEN Werte alles wegschneiden, was nur zeitweise wirkt.
       Sonst landet "Use: Increases attack power by 100 for 20 sec" als
       dauerhafte +100 Angriffskraft in den Stats und das Item wird
       masslos ueberbewertet. Equip-Zeilen bleiben drin, die wirken
       dauerhaft. */
    var statText = text.replace(
      /\b(?:Use|Chance on hit):[^\n]*?(?=\s*(?:Use:|Equip:|Chance on hit:|Requires |Durability |Sell |Classes:|Races:|$))/gi,
      " ");
    ...
    /* ----------------------------------------------------------------
       ...
       16 Angriffskraft, nicht 100. Deshalb werden Wert, Dauer und
       Abklingzeit GETRENNT gespeichert - die Umrechnung macht spaeter
       das Addon, nicht dieser Parser.
    ---------------------------------------------------------------- */
    var effects = [];
    var effRe = /\b(Use|Equip|Chance on hit):\s*([^\n]{4,300}?)(?=\s*(?:Use:|Equip:|Chance on hit:|Requires |Durability |Sell |Classes:|Races:|$))/gi;
    var em;
    while ((em = effRe.exec(text)) !== null) {
      var kind = em[1].toLowerCase().replace(/\s+/g, "");
      var line = em[2].replace(/\s+/g, " ").trim();
      if (!line) { continue; }

      var eff = { kind: kind, text: line };

      var v = line.match(/\bby (?:up to )?(\d+)/i) || line.match(/\b(\d+)\s+(?:to \d+\s+)?(?:damage|health|mana|armor)/i);
      if (v) { eff.value = parseInt(v[1], 10); }

      var dur = line.match(/for (\d+)\s*(sec|min)/i);
      if (dur) {
        eff.duration = parseInt(dur[1], 10) * (/min/i.test(dur[2]) ? 60 : 1);
      }

      effects.push(eff);
    }

    /* Abklingzeit steht meist in einer eigenen Zeile, nicht im Effekttext */
    var cd = text.match(/\((\d+)\s*(Min|Sec)\s*Cooldown\)/i)
          || text.match(/Cooldown:\s*(\d+)\s*(min|sec)/i);
    if (cd && effects.length) {
      var secs = parseInt(cd[1], 10) * (/min/i.test(cd[2]) ? 60 : 1);
      for (var ei = 0; ei < effects.length; ei++) {
        if (effects[ei].kind === "use") { effects[ei].cooldown = secs; }
      }
    }
    ...
    function nextQ() {
      ...
      fetchList("?items=" + filter + mk(q))
        .then(function (rows) {
          ...
          total += rows.length;
          if (rows.length >= CAP) {
            console.warn("[BLL] " + filter + " Qualitaet " + q + ": immer noch "
              + rows.length + " - auch hier fehlen moeglicherweise Items.");
          }
        })
        .catch(function (e) {
